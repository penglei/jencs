...
cs def:richContent(cons) ?><?
	cs def:richContent-items(item) ?><?
		...
		cs elif:item.type == 'url' ?><?
			cs if:item.ourl
				?><a href='http://sns.qzone.qq.com/cgi-bin/qzshare/cgi_qzshare_urlcheck?url=<?cs escape:"url"?><?cs var:item.ourl ?><?cs /escape?>&appid=311&ugcid=<?cs escape:"url"?><?cs var:qz_metadata.t1_tid ?><?cs /escape?>&where=1' title="Ô­Á´½Ó£º<?cs var:item.ourl ?>" class="c_tx" target="_blank"><?cs var:item.url ?></a><?
			cs else
