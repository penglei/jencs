	...
	<?cs else ?>
		...
		<?cs if:qz_metadata.orgdata.mediatype == UC_MEDIA_TYPE_VEDIO && subcount(qz_metadata.orgdata.itemdata) > 0 ?>
			...
			<?cs if:g_isV8 ?>
				...
				<?cs else ?>
					<?cs if:qz_metadata.orgdata.itemdata[0].width>=400 ?>
						<?cs call:set(data_cntmedia_video_show.ret,"videoType", "tencent_big_video") ?>
					<?cs else ?>
						<?cs call:set(data_cntmedia_video_show.ret,"videoType", "leftvideo") ?>
					<?cs /if ?>
				<?cs /if ?>
			<?cs /if ?>
		<?cs #:图片  ?>	
