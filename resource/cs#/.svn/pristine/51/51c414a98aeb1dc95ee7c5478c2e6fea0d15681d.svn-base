...
<?cs def:commentList(comment) ?>
	<?cs def:comment-item(item) ?>
		...
		<qz:reply action="http://wb.qzone.qq.com/cgi-bin/add_reply_ic.cgi" charset="UTF-8" param="topicid=<?cs var:qz_metadata.wbid ?>&amp;parename=<?cs var:qz_metadata.wbaccount ?>&amp;commentid=<?cs var:item.id ?>&amp;tmpl=<?cs if:string.length(qz_metadata.feedstmpl) > 0 ?><?cs var:qz_metadata.feedstmpl ?><?cs else ?>1<?cs /if?>&amp;statfrom=<?cs if:subcount(qz_metadata.rtweet) > 0 ?>2<?cs else ?>1<?cs /if ?>" type="text" version="6" maxLength="140" lengthMode="1" config="0|1|0|0|0|0|0">回复</qz:reply>
	<?cs /def ?>
