...
<?cs def:main() ?>
	<div class="feeds_tp_5">
		...
		<?cs #:操作区 ?>
		<div class="feeds_tp_operate" ocontent="<?cs var:qz_metadata.ocontent ?>" <?cs if:subcount(qz_metadata.rtweet) > 0 ?>rootocontent="<?cs var:qz_metadata.rtweet.ocontent ?> ?>"<?cs /if ?>>
			<qz:reply type="link">评论</qz:reply>
			<qz:popup height="240" width="408" version="" title="分享"
				src="/qzone/app/qzshare/popup.html"
				param="{cgi:'http://wb.qzone.qq.com/cgi-bin/wb_add_share.cgi',fields:{id:'<?cs var:qz_metadata.wbid ?>',type:'',json:1,from:'feeds'<?cs if:subcount(qz_metadata.rtweet) > 0 ?>,rootid:'<?cs var:qz_metadata.rtweet.wbid ?>'<?cs /if ?>,statfrom:<?cs if:subcount(qz_metadata.rtweet) > 0 ?>2<?cs else ?>1<?cs /if ?>}}" 
				>分享
			</qz:popup>
			<qz:popup version="4" charset="utf-8" type="weibo" title="转播到微博" needcontainer="0"
				src="/qzone/app/weibo/script/feedsEditor.js"
				param="id=<?cs var:qz_metadata.wbid ?>&amp;account=<?cs var:qz_metadata.wbaccount ?>&amp;nick=<?cs var:qz_metadata.wbnick ?><?cs if:subcount(qz_metadata.rtweet) > 0 ?>&amp;rootid=<?cs var:qz_metadata.rtweet.wbid ?>&amp;rootaccount=<?cs var:qz_metadata.rtweet.wbaccount ?>&amp;rootnick=<?cs var:qz_metadata.rtweet.wbnick ?><?cs /if ?>"
				>转播到微博
			</qz:popup>
			<a href="http://rc.qzone.qq.com/myhome/weibo/profile/<?cs var:qz_metadata.wbaccount ?>/" class="comment_nickname c_tx" target="_blank" title="查看更多微博">查看更多微博</a>
		</div>
		<?cs #:评论 ?>
