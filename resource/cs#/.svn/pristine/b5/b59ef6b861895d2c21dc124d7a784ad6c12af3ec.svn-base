<?cs if:qz_metadata.privacy==1 || qz_metadata.privacy==4?>
	<?cs if:qz_metadata.edit==1 ?>用照片编辑器<?cs /if ?>的
	<?cs if:qz_metadata.anonymity == 0 ?>相册
		<a class="c_tx" target="_blank" href="http://user.qzone.qq.com/<?cs var:qz_metadata.uin ?>/photo/<?cs var:qz_metadata.albumid ?>/"><?cs var:qz_metadata.albumname ?></a>有了新评论
	<?cs /if ?>
	<?cs if:qz_metadata.anonymity == 1 ?>个性相册：
	...
	<?cs /if ?>
	<?cs if:qz_metadata.anonymity == 3 ?>照片墙：
...
<?cs else ?>
	更新了相册
<?cs /if ?>
<?cs if:qz_metadata.from==22 ?><span class="ifeeds_origin c_tx3">通过手机上网</span><?cs /if ?>