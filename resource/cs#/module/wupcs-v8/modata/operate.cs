...
<?cs def:data_opr_delfeed()?>
	<?cs if:(qz_metadata.meta.loginuin == qz_metadata.meta.opuin) || 
			...
			<?cs call:qfv("operate.delete_feed_opr", 1)?>
		<?cs #else ?>
		<?cs #:没有删除 ?>
	<?cs /if ?>
...
<?cs ####
	/**
	 * 按钮：收藏feed 非被动&&带topicid&&(相应类型的feeds||认证空间feeds)
	 */
?>
<?cs def:data_opr_collect()?>
	<?cs if: qfv.meta.topicid &&
			(qz_metadata.meta.scope != 1) && 
			((qz_metadata.meta.appid == 311 && qz_metadata.meta.typeid != 22) ||
				qz_metadata.meta.appid == 2 ||
				qz_metadata.meta.appid == 202 ||
				qz_metadata.meta.appid == 4 ||
				(string.slice(qz_metadata.meta.otherflag, 0, 1) == '1')
				)
			 ?>
		<?cs set:_path = "operate" ?>
		<?cs call:set(_path, "collect.text", "收藏")?>
	<?cs /if ?>
<?cs /def?>
...
<?cs def:data_opr_more()?>
	...
	<?cs else ?>
		...
		<?cs call:qfv("operate.more_opr.className", "none")?><?cs #先影响，前台用js控制展示出来?>
	<?cs /if ?>
<?cs /def?>
