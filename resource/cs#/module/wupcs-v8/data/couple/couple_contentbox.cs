	...
	<?cs if:qz_metadata.orgdata.mediatype == UC_MEDIA_TYPE_PIC && subcount(qz_metadata.orgdata.itemdata) > 0 ?>
		<?cs if:qz_metadata.meta.feedstype == UC_WUP_FEEDSTYPE_PSV ?>
			<?cs if:subcount(qz_metadata.orgdata.itemdata) >0 ?>
				<?cs if:qz_metadata.orgdata.subtype == COUPLE_srctype_mood ?>
					<?cs set:popup_param = COUPLE_SPACE_OPENID + "|0|" + qz_metadata.orgdata.mkey + "|" 
						+ qz_metadata.orgdata.itemdata[0].itemid + "|" + qz_metadata.orgdata.uin + "|-1|"
						+ qz_metadata.orgdata.itemdata[0].picinfo[0].url + "|" + qz_metadata.orgdata.extendinfo.pairkey 
		...
		<?cs else ?>
			...
			<?cs loop:j=0, _end, 1?>
				<?cs if:qz_metadata.orgdata.subtype == COUPLE_srctype_mood ?>
					<?cs set:popup_param = COUPLE_SPACE_OPENID + "|0|" + qz_metadata.orgdata.mkey + "|" 
						+ qz_metadata.orgdata.itemdata[j].itemid + "|" + qz_metadata.orgdata.uin + "|-1|"
						+ qz_metadata.orgdata.itemdata[j].picinfo[0].url + "|" + qz_metadata.orgdata.extendinfo.pairkey 
						+ "|||" + qz_metadata.orgdata.ctime ?>
				<?cs else ?>
					<?cs set:popup_param = COUPLE_SPACE_OPENID + "|1|" + qz_metadata.orgdata.albumdata.sAlbumId + "|" 
