...
<?cs #:生成feed的id ?>
<?cs def:genFeedId(item) ?><?cs var:item.opuin ?>_<?cs var:item.appid ?>_<?cs var:item.typeid ?>_<?cs var:item.abstime ?>_<?cs var:item.scope ?>_<?cs var:item.ver ?><?cs if:item.specialType ?>_<?cs var:item.specialType ?><?cs /if ?><?cs /def ?>
...
<div class="f_aside imgBlock_img" tabindex="-1">
	<div class="qz_feed_avatar f_user_head">
		<a href="<?cs var:item.userHome ?>" target="_blank" class="q_namecard f_user_avatar f-s-a" link="nameCard_<?cs var:item.opuin ?>">
...
<?cs def:genFeedHTML(item) ?>
	...
	<li class="f_single imgBlock bor2 f-s-s" id="fct_<?cs call:genFeedId(item) ?>">
		<?cs #:头像区域 ?>
