<?cs include:"wupcs/modview/image.cs"?><?cs #默认图片展示，基本接口(压缩、显示)?>
<?cs include:"wupcs/modview/grid_pic.cs"?><?cs #宫格模式图片展示方法?>
<?cs include:"wupcs/modview/gplus_pic.cs"?><?cs #g+(6 + 1)模式展示?>
<?cs include:"wupcs/modview/music.cs" ?>
<?cs include:"wupcs/modview/video.cs" ?>
<?cs include:"wupcs/modview/attachmentPresenter.cs" ?>
<?cs include:"wupcs/modview/attachViewer.cs" ?>
<?cs include:"wupcs/modview/voice_player.cs" ?>
<?cs include:"wupcs/modview/smart_pic.cs"?>
...
<?cs def:_contentMedia_pictures()?>
	...
	<?cs if:_imgMode == G_IMG_GPLUS_MODE?><?cs #六+1模式?>
		<?cs call:contentMedia_pic_GPlus()?>
	<?cs elif:_imgMode == G_IMG_GRID_MODE?><?cs #多宫格模式展示?>
		<?cs call:contentMedia_pic_Grid("big")?>
	<?cs elif:_imgMode == G_IMG_SMALL_MODE?><?cs #小图模式?>
		<?cs #小图模式是 120 * 120，按长边压缩的模式?>
		<?cs call:_limit_image_size(120, 120)?>
		<?cs call:contentMedia_pic_default()?><?cs #默认的样子?>
	<?cs elif:_imgMode == G_IMG_SMALL_V8_MODE?><?cs #v8小图模式?>
		<?cs call:contentMedia_pic_LeftSmallPic() ?>
	<?cs elif:_imgMode == G_IMG_GRID_MODE_SMALL?><?cs #小图多宫格模式展示?>
		<?cs call:contentMedia_pic_Grid("small") ?>
	...
	<?cs else ?>
		<?cs if:qfv.meta.feedstype == UC_WUP_FEEDSTYPE_PSV?>
			<?cs call:_limit_image_size(120, 120)?>
		<?cs else ?>
			<?cs call:_limit_image_size(400, 400)?>
		<?cs /if?>
		<?cs call:contentMedia_pic_default()?><?cs #默认的样子?>
	<?cs /if?>
<?cs /def?>
...
<?cs def:_contentMedia_videos()?>
	...
	<?cs if:subcount(qfv.content.media.video)?>
		<?cs call:video(qfv.content.media.video.0)?>
	<?cs /if?>
<?cs /def?>
...
<?cs def:contentMedia()?>
	...
	<?cs if:subcount(qfv.content.voice)?><?cs #语音可以和图片一起展示?>
		<?cs call:voice_player(qfv.content.voice) ?>
	<?cs /if?>
	...
	<?cs elif:subcount(qfv.content.attach)?>
