...
<?cs def:data_vote_title()?>
	...
	<?cs if:qz_metadata.feedtype == UC_WUP_FEED_TYPE_ACT || qz_metadata.feedtype == UC_WUP_FEED_TYPE_RELATEPSV?>
		...
		<?cs if:string.length(qz_metadata.votedata.join_body_uin) > 0 && qz_metadata.votedata.join_body_uin != VOTE_INVALID_JOIN_UIN?>
		...
		<?cs if:string.length(qz_metadata.votedata.join_body_uin) > 0 && qz_metadata.votedata.join_body_uin != VOTE_INVALID_JOIN_UIN?>
			...
			<?cs set:_opt_cnt = ""?>
			<?cs set:_end = subcount(qz_metadata.votedata.join_body_option) - 1?>
			<?cs loop:j=0, _end, 1?>
				<?cs set:_opt_cnt = _opt_cnt + qz_metadata.votedata.join_body_option[j]?>
				<?cs if:j != _end?>
					<?cs set:_opt_cnt = _opt_cnt + "、"?>
				<?cs /if?>
			<?cs /loop?>
			<?cs if:qz_metadata.votedata.join_count > 0?>
				<?cs set:_opt_cnt = _opt_cnt + "，超过" + qz_metadata.votedata.join_count + "人参与该投票"?>
			<?cs /if?>
	...
	<?cs elif:qz_metadata.feedtype == UC_WUP_FEED_TYPE_ATMEPSV_BY_COM || qz_metadata.feedtype == UC_WUP_FEED_TYPE_ATMEPSV_BY_REPLY 
			|| qz_metadata.feedtype == UC_WUP_FEED_TYPE_ATMEPSV?>
		<?cs call:data_title_tipTxt("在投票中提到了我")?>
	<?cs elif:qz_metadata.feedtype == UC_WUP_FEED_TYPE_ACT_NOTIFYPSV_CUR?>
		<?cs set:_opt_cnt = ""?>
