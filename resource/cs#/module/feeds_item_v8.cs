...
<?cs #:生成feed的id ?>
<?cs def:v8_genFeedId(item) ?><?cs var:item.opuin ?>_<?cs var:item.appid ?>_<?cs var:item.typeid ?>_<?cs var:item.abstime ?>_<?cs var:item.scope ?>_<?cs var:item.ver ?><?cs if:item.specialType ?>_<?cs var:item.specialType ?><?cs /if ?><?cs /def ?>
...
<?cs ####
	 ...
	 */
?>
<?cs def:v8_genTitleStart(item)?>
	<?cs set:qz_metadata.hdf.yybitmap=item.yybitmap?><?cs #/*feed_title_icon.cs是给feeds模版用的，不应该在这里调用的!*/?>
	<?cs #:item.nofold  代表是否是客人首页或个人档 ?>
	<?cs if:item.scope != 1 && item.scope != 100?>
		<div class="f-nick">
			<?cs call:v8_setEmoji(item.emoji) ?>
			<a target="_blank" href="<?cs var:item.userHome ?>" class="f-name q_namecard" link="nameCard_<?cs var:item.opuin ?>">
...
<?cs ####
	/*feed头像 f-aside*/
	function v8_feed_avatar(item){}
?>
<?cs def:v8_feed_avatar(item) ?>
<div class="f-aside" tabindex="-1">
	<a href="<?cs var:item.userHome ?>" target="_blank" class="f-user-avatar q_namecard f-s-a" link="nameCard_<?cs var:item.opuin ?>">
		<img alt="" src="<?cs alt:item.logimg ?>/ac/b.gif<?cs /alt ?>">
	</a>
			...
			>
				...
				<?cs if:isWupfeed != 1?>
					...
					<?cs elif:string.length(item.title)>0 ?>
						</div><?cs #/*闭合v8_genTitleStart里的div*/?> 
					<?cs /if ?>
