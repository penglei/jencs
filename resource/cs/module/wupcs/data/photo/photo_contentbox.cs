...
<?cs ####
	/*转载相片会有括号内容*/
?>
<?cs def:data_photo_quote()?>
	<?cs call:get_photo_current_picindex() ?>
	
	<?cs #单图类的描述展示current pic对应的描述，多图类统一使用第一张描述 ?>
	<?cs if:qz_metadata.feedtype==UC_WUP_FEED_TYPE_NEWCOMMENT || qz_metadata.meta.typeid==FEED_TYPE_PHOTO || qz_metadata.meta.typeid==FEED_ABOUT_PHOTO || qz_metadata.meta.typeid==FEED_TYPE_PHOTOCMT || qz_metadata.meta.typeid==FEED_TYPE_AT_PHOTO?>
		<?cs call:data_quote_desc(qz_metadata.orgdata.itemdata[get_photo_current_picindex.ret].desc)?>
	<?cs else ?>
...
<?cs def:data_photo_lovekid() ?>
	<?cs if:string.length(qz_metadata.orgdata.albumdata.extendinfo.sAge) >0  && string.length(qz_metadata.orgdata.albumdata.iPicNum)>0 ?>
		<?cs set:qfv.titleExtend = 1 ?>
		<?cs set:qfv.titleExtend.extendinfo = qz_metadata.orgdata.albumdata.extendinfo.sAge ?>
		<?cs set:qfv.titleExtend.url =  "http://user.qzone.qq.com/" + qz_metadata.orgdata.uin + "/photo/" 
						+ qz_metadata.orgdata.albumdata.sAlbumId ?> 
		<?cs set:qfv.titleExtend.num = qz_metadata.orgdata.albumdata.iPicNum ?>
		<?cs set:qfv.titleExtend.albumName = qz_metadata.orgdata.albumdata.sAlbumName?>
		<?cs set:qfv.titleExtend.type = qz_metadata.orgdata.albumdata.iAnonymity ?>
	<?cs /if ?>
<?cs /def ?>
...
<?cs def:data_photo_contentbox()?>
	...
	<?cs if:qz_metadata.feedtype == UC_WUP_FEED_TYPE_COMMPSV || qz_metadata.feedtype == UC_WUP_FEED_TYPE_REPLYPSV || qz_metadata.feedtype == UC_WUP_FEEDS_TYPE_SHARETOME || qz_metadata.feedtype == UC_WUP_FEED_TYPE_AUDIT || qz_metadata.feedtype == UC_WUP_FEED_TYPE_ACT_NOTIFYPSV?>
