<?cs #{/*需要使用wup元数据
	...
	*/
?>
<?cs def:v8_echoUsername(user, defaultName)?>
...
<?cs def:v8_userLink_comp(user)?>
	...
	>
		...
		<?cs call:v8_echoUsername(user, user.nickname)?>
	</a>
	<?cs /if?>
<?cs /def?>

<?cs def:v8_authSpaceIcon_comp()?>
...
<?cs def:v8__userAvatar_url(user, size)?>
	...
	<?cs if:user.who == USER_PLATFORM_WHO_QZONE || user.who == USER_PLATFORM_WHO_WEIBO?>
		<?cs if:string.length(qz_metadata["logoPool" + size]['u' + user.uin])>0 ?>
			<?cs set:v8__userAvatar_url.ret = qz_metadata["logoPool" + size]['u' + user.uin] ?>
		<?cs else ?>
			<?cs set:_user_icon_mod = user.uin % 4 + 1?>
			<?cs set:v8__userAvatar_url.ret = "http://qlogo" + _user_icon_mod +
											".store.qq.com/qzone/" +
											user.uin + "/" + user.uin + "/" + size?>
		<?cs /if?>
	<?cs elif:user.who == USER_PLATFORM_WHO_PY ?>
		<?cs if:string.length(qz_metadata["xylogoPool" + size]['u' + user.uin]) > 0 ?>
			<?cs set:v8__userAvatar_url.ret = qz_metadata["xylogoPool" + size]['u' + user.uin]?>
		<?cs elif:user.img ?>
			<?cs set:v8__userAvatar_url.ret = "http://xy.store.qq.com/campus/" + user.img + "/" + size?>
		<?cs else ?>
			<?cs set:v8__userAvatar_url.ret = "http://py.qlogo.cn/friend/" + user.uin + "/audited/" + size?>
		<?cs /if?>
	<?cs /if?>
<?cs /def?>

<?cs def:v8__userAvatar_url_by_uin(uin, size)?>
	<?cs if:!size?><?cs set:size = 30?><?cs /if?>
	<?cs if:string.length(qz_metadata["logoPool" + size]['u' + uin])>0 ?>
		<?cs set:v8__userAvatar_url_by_uin.ret = qz_metadata["logoPool" + size]['u' + uin] ?>
	<?cs else ?>
		<?cs set:_user_icon_mod = uin % 4 + 1?>
		<?cs set:v8__userAvatar_url_by_uin.ret = "http://qlogo" + _user_icon_mod + ".store.qq.com/qzone/" + uin + "/" + uin + "/" + size?>
	<?cs /if?>
<?cs /def?>
...
<?cs ####
	 ...
	 */
?>
<?cs def:v8_userAvatar_comp(user, size)?>
	...
	<?cs if:user.who == USER_PLATFORM_WHO_QZONE || user.who == USER_PLATFORM_WHO_WEIBO?><?cs #/*QZONE用户，微博用户的头像也用空间的*/?>
		<a href="<?cs call:v8_echoUserlink(user)?>" target="_blank">
	...
	<?cs elif:user.who == USER_PLATFORM_WHO_PY ?><?cs #/*朋友用户*/?>
		<a href="<?cs call:v8_echoUserlink(user)?>" class="c_tx" target="_blank">
			<img alt="<?cs call:v8_echoUsername(user, user.nickname)?>" src="<?cs var:v8__userAvatar_url.ret ?>" />
		</a>
	<?cs /if ?>
<?cs /def?>
