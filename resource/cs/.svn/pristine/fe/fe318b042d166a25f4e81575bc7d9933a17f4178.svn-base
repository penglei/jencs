...
<?cs def:data_textTitle_txtPopup(text, title, src, param, version, width, height)?>
	<?cs set:_path = _cnt_summary_title_path + textTitle_i?>
	<?cs call:data_con_txt(_path, text, "", 10)?>
	<?cs call:data_popup(
			_path + ".action",
			title, src, param, version, width, height, "", "")?>
	<?cs set:data_textTitle_txtPopup.ret.path = _path?>
	<?cs call:i++()?>
<?cs /def?>
