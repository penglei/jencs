...
<?cs def:richContent-title(cons) ?>
	<?cs def:richContentTitle-items(item) ?>
		...
		<?cs elif:item.type == 'url' ?>
			<a class="c_tx ui_mr5 " href="<?cs call:ugc_url_check(item.url,1) ?>" target="_blank"><?cs if:item.text ?><?cs var:item.text ?><?cs else ?><?cs var:item.url ?><?cs /if ?></a>
...
<?cs def:richTitle(cons) ?>
	...
	<?cs def:richTitle-items(item, i, length) ?>
		...
		<?cs elif:item.type == 'url' ?>
			<?cs if:qz_richtitle_flag == 1 ?>
				</span><a class="c_tx ui_mr5" href="<?cs call:ugc_url_check(item.url,1) ?>" target="_blank"><?cs var:item.text ?></a>
				<?cs set:qz_richtitle_flag = 0 ?>
			<?cs elif:qz_richtitle_flag == 0 ?>
				<a class="c_tx ui_mr5" href="<?cs call:ugc_url_check(item.url,1) ?>" target="_blank"><?cs var:item.text ?></a>
			<?cs /if ?>
...
<?cs def:actionTitle(mod) ?>
	...
	<?cs def:actionTitle_url(mod) ?>
		...
		<?cs /if ?>
		<?cs if:url_url ?>
			<a class="c_tx ui_mr5" href="<?cs call:ugc_url_check(url_url,1) ?>" target="_blank"><?cs var:url_text ?></a>
		<?cs /if ?>
	<?cs /def ?>	
...
<?cs def:modCaller(titleMod) ?>
	...
	<?cs if:qz_metadata.metadata.appid == 4?>
		<?cs if:qz_metadata.metadata.privacy == 6 ?><?cs #指定好友可见?>
			<span class="ui_mr5 c_tx3">【指定好友可见】</span>
		<?cs elif:qz_metadata.metadata.privacy == 2 || qz_metadata.metadata.privacy == 5?>
			<span class="ui_mr5 c_tx3">【已回答过问题可见】</span>
		<?cs /if?>
...
<?cs if:subcount(qz_metadata.actiontitle.con_more)>0 ?>
	<p class="info_cut"><?cs call:start("title")?></p>
	<p class="info_complete none"></p>
	<div class="f_toggle">
		<a href="javascript:;" data-cmd="qz_toggle" data-action="<?cs var:qz_metadata.actiontitle.con_more.action ?>" data-param="<?cs var:qz_metadata.actiontitle.con_more.param ?>" data-complete=0 data-isshuoshuo=1>展开查看全文</a>
