	...
	<?cs if:qz_metadata.albumid ?>
		...
		<div class=" feeds_tp_1">
			...
			<div class="feeds_tp_operate">
				<qz:popup title="添加照片到我的分享" param="3|<?cs var:qz_metadata.albumid ?>$||^<?cs var:qz_metadata.albumname_en ?>$||^<?cs var:qz_metadata.lloc ?>$||^<?cs var:qz_metadata.zzuin ?>$||^<?cs var:qz_metadata.zzphotoname_en ?>$||^<?cs var:qz_metadata.photodesc_en ?>$||^<?cs var:qz_metadata.zznick_en ?>$||^<?cs var:qz_metadata.smallurl_en ?>$||^<?cs var:qz_metadata.bigurl_en ?>" src="/qzone/photo/zone/sharecanvas.html" width="432" height="300">
						分享
				</qz:popup>
				<qz:popup title="转载到我的相册" param="<?cs var:qz_metadata.zzuin ?>$||^<?cs var:qz_metadata.zzphotoname ?>$||^<?cs var:qz_metadata.photodesc ?>$||^<?cs var:qz_metadata.zzuin ?>$||^<?cs var:qz_metadata.phototype ?>$||^<?cs var:qz_metadata.lloc ?>$||^<?cs var:qz_metadata.sloc ?>$||^<?cs var:qz_metadata.owneruin ?>$||^<?cs var:qz_metadata.ownernick ?>$||^<?cs var:qz_metadata.albumid ?>$||^<?cs var:qz_metadata.photocubage ?>$||^<?cs var:qz_metadata.width ?>$||^<?cs var:qz_metadata.height ?>$||^<?cs var:qz_metadata.bigurl_en ?>$||^<?cs var:qz_metadata.zzphotoname ?>$||^<?cs var:qz_metadata.photodesc ?>" src="/qzone/photo/zone/icenter_reprint.html" width="470" height="265">
					转载
				</qz:popup>
			</div>
				...
				<div class="feeds_comment">
					...
					<?cs if:qz_metadata.omit>0 ?>
						<div class="more_feeds_comment bg2">
							<a class="c_tx" href="http://user.qzone.qq.com/<?cs var:qz_metadata.zzuin ?>/photo/<?cs var:qz_metadata.albumid ?>/<?cs var:qz_metadata.lloc ?>/" target="_blank">查看全部<?cs var:qz_metadata.total ?>条评论>></a>
						</div>
					<?cs /if ?>
