...
<?cs def:oprLike()?>
	<?cs if:subcount(qfv.like) > 0?>
		<a href="javascripr:;" class="ui_btn op_btn_like hotclick" 
...
<?cs def:oprComment()?>
	<?cs if:subcount(qfv.operate.comment_btn) > 0?>
		<a href="javascripr:;" 
			class="ui_btn op_btn_comment hotclick" 
			data-role="commentBtn" 
			data-hctag="spcaretab.comment.click"><i class="ui_icon icon_op_comment"></i>评</a>
	<?cs /if?>
<?cs /def?>
...
<?cs def:oprForward()?>
	<?cs if:subcount(qfv.operate.forward)?>
		<a href="javascripr:;" class="ui_btn op_btn_rt hotclick" data-hctag="spcaretab.share.click" data-fwddenied=""><i class="ui_icon icon_op_rt"></i>转</a>
	<?cs /if?>
<?cs /def?>
...
<?cs def:operate() ?>
	<?cs if:subcount(qfv.operate) > 0?>
		<p class="f-detail">
			<a href="javascripr:;" class="item op_btn_like hotclick" 
				data-likecnt="<?cs var:qfv.like.count?>" 
				data-showcount="<?cs var:qfv.like.likeinfo?>" 
				data-hctag="spcaretab.<?cs if:qfv.like.isliked?>unlike<?cs else ?>like<?cs /if ?>.click">
			...
			<a href="javascripr:;" class="item op_btn_comment hotclick" data-role="commentBtn" <?cs if:qfv.comments.inputbox.useReply==1 ?>data-typeid="1" <?cs /if?>data-hctag="spcaretab.comment.click"><i class="ui-icon icon-comment"></i>
				<?cs var:html_encode(qfv.operate.comment_btn.text, 1)?>
			</a>
			<span class="item-line"></span>
			<a href="javascripr:;" class="item op_btn_rt hotclick" data-hctag="spcaretab.share.click"<?cs if:subcount(qfv.operate.privacy_icon)?> data-fwddenied="1"<?cs /if?>><i class="ui-icon icon-forward"></i>转发
				<?cs if:qfv.operate.forward.count>0?>
					(<?cs var:qfv.operate.forward.count?>)
				<?cs /if?>
			</a>
			<?cs #/*只有被动才有删除按钮*/?>
			<?cs if:qz_metadata.meta.feedstype == UC_WUP_FEEDSTYPE_PSV?>
			<span class="item-line"></span>
			<a href="javascript:;" class="item op_btn_delete hotclick" data-hctag="spcaretab.del.click"><i class="ui-icon icon-delete"></i>删除</a>
			<?cs /if?>
		</p>		
	<?cs /if?>
<?cs /def ?>
