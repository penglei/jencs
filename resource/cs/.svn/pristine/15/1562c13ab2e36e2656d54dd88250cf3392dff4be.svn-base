	...
	<?cs if:qz_metadata.meta.feedstype == UC_WUP_FEEDSTYPE_PSV ?>
		...
		<?cs if:subcount(qz_metadata.relybody) > rely_pos ?>
			...
			<?cs if:subcount(qz_metadata.relybody[rely_pos].msg) > 0 && 
					(subcount(qz_metadata.relybody[rely_pos].msg) > 1 ||
						qz_metadata.relybody[rely_pos].msg[0].type != 0 ||
						string.length(qz_metadata.relybody[rely_pos].msg[0].content) > 0
					)
			?>
...
<?cs def:data_mood_contentbox()?>
	...
	<?cs if: qz_metadata.meta.feedstype == UC_WUP_FEEDSTYPE_PSV ?>
		...
		<?cs else ?>
			...
			<?cs elif:_g_forward == 1 && subcount(qz_metadata.orgdata.itemdata)?>
				<?cs call:data_content_init(G_LAYOUT_LEFTIMG_V8, G_IMG_DEFAULT, "") ?>
			<?cs else ?>
				<?cs call:data_content_init(G_LAYOUT_DEFAULT , G_IMG_DEFAULT , "") ?>
			<?cs /if ?>
		<?cs /if ?>
	<?cs else ?>
		<?cs  set:_smartlayouttypeLength = string.length(qz_metadata.orgdata.extendinfo.smartlayouttype) ?>
		<?cs if: _smartlayouttypeLength > 0 && 
			subcount(SMART_LAYOUT[qz_metadata.orgdata.extendinfo.smartlayouttype]) > 0?>
			<?cs call:data_content_init(G_LAYOUT_DEFAULT,G_IMG_SMART , "") ?>
			<?cs call:set("content.media","smartlayouttype", qz_metadata.orgdata.extendinfo.smartlayouttype) ?>
			<?cs set:_smartlayouttype = qz_metadata.orgdata.extendinfo.smartlayouttype ?>
		<?cs else ?>
			<?cs call:data_content_init(G_LAYOUT_DEFAULT,G_IMG_GRID_MODE  , "") ?>
		<?cs /if ?>
	<?cs /if ?>
	...
	<?cs if:qz_metadata.orgdata.mediatype == UC_MEDIA_TYPE_VEDIO && subcount(qz_metadata.orgdata.itemdata) > 0 ?>
		...
		<?cs call:data_cntmedia_video_show(0, qz_metadata.orgdata.itemdata[0], "3&videosrc="+uri_encode(qz_metadata.orgdata.itemdata[0].vidioswfurl)+"&type="+qz_metadata.orgdata.itemdata[0].type, play_param, "") ?>
	<?cs elif:qz_metadata.orgdata.mediatype == UC_MEDIA_TYPE_PIC && subcount(qz_metadata.orgdata.itemdata) > 0 ?>
		...
		<?cs else?>
			...
			<?cs set:popup_param = get_tuin_and_tid.tid + "|" + get_tuin_and_tid.uin + "|" + j?>
			<?cs if:string.length(_smartlayouttype) <= 0 ?>
				<?cs call:data_cntmedia_pic_popup(j, qz_metadata.orgdata.itemdata[j], popup_param, "/qzone/photo/zone/icenter_popup.html", "") ?>
			<?cs else ?>
				<?cs call:data_cntmedia_pic_popup_v3(j, qz_metadata.orgdata.itemdata[j], popup_param, "/qzone/photo/zone/icenter_popup.html", "",
					SMART_LAYOUT[_smartlayouttype][j+1].width,SMART_LAYOUT[_smartlayouttype][j+1].height) ?>
			<?cs /if ?>
			<?cs set:_pickey = qz_metadata.orgdata.itemdata[j].picinfo.0.extendinfo.mood_pickey?>
			<?cs if:!_is_in_qqtab && _pickey ?>
				<?cs #新版浮层需要的参数 ?>
				...
				<?cs #:单独统计版式点击统计 ?>
				<?cs if:string.length(_smartlayouttype) >0 ?>
					<?cs call:data_popup_add_attr(_actionpath, "hotclickPath", "0_appid_311.pic_count_" + _count + ".pic_" + j + "." + _smartlayouttype) ?>
				<?cs else ?>
