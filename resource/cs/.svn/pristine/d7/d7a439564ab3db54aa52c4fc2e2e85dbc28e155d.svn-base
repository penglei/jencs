	...
	<?cs if:subcount(qz_metadata.comments) > 0 && qz_metadata.comments.count > 0 && subcount(qz_metadata.comments.item) > 0 ?>
		<?cs if:subcount(qz_metadata.comments) > 0 && qz_metadata.comments.count > 2 ?>
                        <div class="more_feeds_comment bg2">
                            <a target="_blank" href="http://rc.qzone.qq.com/myhome/<?cs var:qz_metadata.qzone_app_ww ?>/<?cs var:qz_metadata.qzone_app_ww_question ?>/<?cs var:qz_metadata.qid ?>/" class="c_tx">查看全部<?cs var:qz_metadata.comments.count ?>条回答&gt;&gt;
                            </a>
                        </div>
		<?cs /if ?>
