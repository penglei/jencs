...
<p class="img_amount c_tx3">
	<?cs if:string.length(bbssource) > 0 ?>  来自<a href="<?cs var:url ?>" target="_blank" class="c_tx3"><?cs call:html_escape(bbssource) ?></a>&nbsp;
	<?cs else?>
		<?cs if: type>0&&type<4 ?>
			<?cs if: fromtype==200 ?>
	...
	空间<?cs var:feedstypename?>(<?cs var:sharecount?>)
</p>
...
<div class="bor2 mood_rt_frame bgr3">
	<p class="mood_rt_text">
		<span class="c_tx1"><?cs if:subcount(qz_metadata.summary_wb)>0?><?cs call:richSummary(qz_metadata.summary_wb) ?><?cs else?><?cs var:qz_metadata.summary ?><?cs /if?></span>
	</p>
...
<div class="bor2 mood_rt_frame bgr3">
	<strong>
		<p class="mood_rt_text">
			<span class="c_tx1">
				<qz:popup param="{action:3,songid:<?cs var:resid?>,flashid:'flash_<?cs var:itemid?>',playbtn:'controls_<?cs var:itemid?>',redarea:'player_<?cs var:itemid?>' <?cs if:typename=='高品质音乐'?>,quality:1<?cs /if?>}"
	...
	<div id="player_<?cs var:itemid?>" style="display:none;">
		<qz:popup param="{action:4,songid:<?cs var:resid?>,flashid:'flash_<?cs var:itemid?>',playbtn:'controls_<?cs var:itemid?>',redarea:'player_<?cs var:itemid?>' <?cs if:typename=='高品质音乐'?>,quality:1<?cs /if?>}" 
...
<div class="feeds_tp_3 music_player_feed">
	...
	<div class="feeds_tp_operate">
		...
		<a href="<?cs if:type==4 || type==15 ?>http://sns.qzone.qq.com/cgi-bin/qzshare/cgi_qzshare_urlcheck?uin=<?cs var:uin ?>&shareid=<?cs var:itemid ?><?cs else ?>http://user.qzone.qq.com/<?cs var:uin ?>/share/<?cs var:itemid ?><?cs /if ?>" class="c_tx" target="_blank"><span class="c_tx">查看详情</span></a>
	</div>
