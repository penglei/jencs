...
unsigned long long var_ull_int_lookup (CSPARSE *parse, char *name)
{
	char *vs;
...
char *arg_eval (CSPARSE *parse, CSARG *arg)
{
	switch ((arg->op_type & CS_TYPES))
	{
		...
		default:
			ne_warn ("Unsupported type %s in arg_eval,line=%d", expand_token_type(arg->op_type, 1), __LINE__);
			return NULL;
	}
}

/* This coerces everything to numbers */
unsigned long long arg_eval_num_ui (CSPARSE *parse, CSARG *arg)
{
	...
	switch ((arg->op_type & CS_TYPES))
	{
		...
		case CS_TYPE_VAR:
		case CS_TYPE_VAR_NUM:
			v = var_ull_int_lookup (parse, arg->s);
			break;
		default:
			ne_warn ("Unsupported type %s in arg_eval_num, line=%d", expand_token_type(arg->op_type, 1), __LINE__);
			v = 0;
			break;
	}
	return v;
...
/* This coerces everything to numbers */
unsigned long long arg_eval_num_ull (CSPARSE *parse, CSARG *arg)
{
	unsigned long long v = 0;
...
long int arg_eval_bool (CSPARSE *parse, CSARG *arg)
{
	...
	switch ((arg->op_type & CS_TYPES))
	{
		...
		case CS_TYPE_VAR_NUM: /* this implies forced numeric evaluation */
			...
			break;
		default:
			ne_warn ("Unsupported type %s in arg_eval_bool,line=%d", expand_token_type(arg->op_type, 1), __LINE__);
			v = 0;
			break;
	}
	return v;
...
char *arg_eval_str_alloc (CSPARSE *parse, CSARG *arg)
{
	...
	switch ((arg->op_type & CS_TYPES))
	{
		...
		case CS_TYPE_VAR_NUM:
			...
			break;
		default:
			ne_warn ("Unsupported type %s in arg_eval_str_alloc, line=%d",
					expand_token_type(arg->op_type, 1), __LINE__);
			s = NULL;
			break;
	}
...
static NEOERR *call_eval (CSPARSE *parse, CSTREE *node, CSTREE **next)
{
	...
	for (x = 0; x < macro->n_args; x++)
	{
		...
		{
			ne_warn("Unsupported type %s in call_expr,line=%d", expand_token_type(val.op_type, 1), __LINE__);
		}
		if (val.alloc) free(val.s);
...
static int cal_bitmap_result_value_ex(unsigned long long value, int start, int num)
{
	start --;  //使用时，是 1-64 位，这里处理时是 0-63

	if(start < 0 || start > 63 || num > 64)
	{
		return 0;
	}
...
static int cal_bitmap_result_value(unsigned long long value)
{
	int t = 64;
	while((value & 0x01) == 0 && t--)
	{
		value >>= 1;
	}
	return (int)value;
...
static int HtmlEncode(const char *s, char **ans, int flag)
{
	int srclen = strlen(s);
	int dstlen = srclen * 6 + 10;
	if(((*ans) = malloc(dstlen)) == NULL)
	{
		return -1;
	}
	/*make sure that pointer *ans and s is not null!*/
	escapeXHTMLEntity4CS(*ans, s, dstlen, PRO_SPACE_NO, flag);
	return 0;
}
static int HtmlDecode(const char *s, char **ans, int flag)
{
	int srclen = strlen(s);
	int dstlen = srclen + 10;
	if(((*ans) = malloc(dstlen)) == NULL)
	{
		return -1;
	}
	/*make sure that pointer *ans and s is not null!*/
	unescapeXHTMLEntity4CS(*ans, s, dstlen, PRO_SPACE_NO, flag);
	return 0;
}
static int JsonEncode(const char *s, char **ans, int flag)
{
	int srclen = strlen(s);
	int dstlen = srclen * 2 + 10;
	if(((*ans) = malloc(dstlen)) == NULL)
	{
		return -1;
	}
	/*make sure that pointer *ans and s is not null!*/
	escapeCString4CS(*ans, s, dstlen, flag);
	return 0;
}
static int JsonDecode(const char *s, char **ans, int flag)
{
	int srclen = strlen(s);
	int dstlen = srclen + 10;
	if(((*ans) = malloc(dstlen)) == NULL)
	...
	/*make sure that pointer *ans and s is not null!*/
	unescapeCString4CS(*ans, s, dstlen, flag);
	return 0;
}
static int UriEncode(const char *s, char **ans)
{
	int srclen = strlen(s);
	int dstlen = srclen * 3 + 10;
	if(((*ans) = malloc(dstlen)) == NULL)
	{
		return -1;
	}
	/*make sure that pointer *ans and s is not null!*/
	encodeURIValue4CS(*ans, s, dstlen);
	return 0;
}
static int UriDecode(const char *s, char **ans)
{
	int srclen = strlen(s);
	int dstlen = srclen + 10;
	if(((*ans) = malloc(dstlen)) == NULL)
	...
	/*make sure that pointer *ans and s is not null!*/
	decodeURIValue4CS(*ans, s, dstlen);
	return 0;
}
static int UrlReplace4Py(const char *s, char **ans)
{
	if(s == NULL)
	{
		return -1;
	...
	char * ptrHostBegin = NULL;
	char * ptrHostEnd = NULL;
	char * ptrUinBegin = NULL;
	char * ptrUinEnd= NULL;

	//找到 http://user.qzone.qq.com/412985113/blog 的 u
	int t= 0;
	ptrHostBegin = (char *)s;
	while(*ptrHostBegin != '\0' && *ptrHostBegin != 'u' && t < 10)
	{
		ptrHostBegin ++;
		t ++;
	}
	if(*ptrHostBegin != 'u')
	{
		goto ERR_RET_NO_CHANGE;
	}

	//找到 http://user.qzone.qq.com/412985113/blog 的 com 后面的 /
	t = 0;
	ptrHostEnd = ptrHostBegin;
	while(*ptrHostEnd != '\0' && *ptrHostEnd != '/' && t < 20)
	{
		ptrHostEnd ++;
		t ++;
	}
	if(*ptrHostEnd != '/' || (ptrHostEnd - s) > (len - 3))
	{
		goto ERR_RET_NO_CHANGE;
	}

	//找到 http://user.qzone.qq.com/412985113/blog 的 4
	ptrUinBegin = ptrHostEnd + 1;
	if(*ptrUinBegin < '1' || *ptrUinBegin > '9')
	{
		goto ERR_RET_NO_CHANGE;
	}

	//找到 http://user.qzone.qq.com/412985113/blog 的 3 后面的 /
	t = 0;
	ptrUinEnd = ptrUinBegin;
	while(*ptrUinEnd != '\0' && *ptrUinEnd != '/' && t < 11)
	{
		ptrUinEnd ++;
		t ++;
	}
	...
	//printf("POS: %d %d %d %d\n", ptrHostBegin - s, ptrHostEnd - s, ptrUinBegin- s, ptrUinEnd - s);

	//取UIN 加密
	char uinBuf[15];
	strncpy(uinBuf, ptrUinBegin, (ptrUinEnd - ptrUinBegin));
	char strID[C_QQ_ENCODE_MAXLEN+1] = {0};
	unsigned int encodlen = C_QQ_ENCODE_MAXLEN;
	cs_encode_qq((char*)uinBuf, (char*)"Bt4e2%Y#X@~g.+F<", (char*)strID, &encodlen);

	//开结果buffer
	if(((*ans) = malloc(len + 100)) == NULL)
	{
		return -1;
	}
	memset(*ans, 0, sizeof(len + 100));

	char * ptrResult = *ans;
	strncpy(ptrResult, s, (ptrHostBegin - s));
	ptrResult += (ptrHostBegin - s);
	//printf("ADDLEN %d\n", ptrHostBegin - s);

	strncpy(ptrResult, "baseapp.pengyou.com/", 20);
	ptrResult += 20;
	//printf("ADDLEN %d\n", 20);

	strncpy(ptrResult, strID, encodlen);
	ptrResult += encodlen;
	//printf("ADDLEN %d\n", encodlen);

	strncpy(ptrResult, ptrUinEnd, (len - (ptrUinEnd - s)));
	ptrResult += (len - (ptrUinEnd - s));
	//printf("ADDLEN %d\n", len - (ptrUinEnd - s));
	*ptrResult = '\0';
...
static NEOERR * _builtin_html_decode(CSPARSE *parse, CS_FUNCTION *csf, CSARG *args,
		CSARG *result)
{
	NEOERR *err;
	char *s = NULL;
	...
	err = cs_arg_parse(parse, args, "si", &s, &flag);
	if (err) {
		return nerr_pass(err);
	}
...
static NEOERR * _builtin_html_encode(CSPARSE *parse, CS_FUNCTION *csf, CSARG *args,
		CSARG *result)
{
	NEOERR *err;
	char *s = NULL;
...
static NEOERR * _builtin_uri_decode(CSPARSE *parse, CS_FUNCTION *csf, CSARG *args,
		CSARG *result)
{
	NEOERR *err;
	char *s = NULL;
...
static NEOERR * _builtin_url_replace_4py(CSPARSE *parse, CS_FUNCTION *csf, CSARG *args,
		CSARG *result)
{
	NEOERR *err;
	char *s = NULL;
...
static NEOERR * _builtin_uri_encode(CSPARSE *parse, CS_FUNCTION *csf, CSARG *args,
		CSARG *result)
{
	NEOERR *err;
	char *s = NULL;
...
static NEOERR * _builtin_json_decode(CSPARSE *parse, CS_FUNCTION *csf, CSARG *args,
		CSARG *result)
{
	NEOERR *err;
	char *s = NULL;
...
static NEOERR * _builtin_json_encode(CSPARSE *parse, CS_FUNCTION *csf, CSARG *args,
		CSARG *result)
{
	NEOERR *err;
	char *s = NULL;
...
static NEOERR * _builtin_bit_or(CSPARSE *parse, CS_FUNCTION *csf, CSARG *args,
		CSARG *result)
{
	NEOERR *err;
	long int n1 = 0;
...
static NEOERR * _builtin_u32_mod(CSPARSE *parse, CS_FUNCTION *csf, CSARG *args,
		CSARG *result)
{
	NEOERR *err;
	unsigned int n1 = 0;
	...
	err = cs_arg_parse(parse, args, "uu", &n1, &n2);
	if (err)
	{
		return nerr_pass(err);
	}
...
static NEOERR * _builtin_bit_and(CSPARSE *parse, CS_FUNCTION *csf, CSARG *args,
		CSARG *result)
{
	NEOERR *err;
	long int n1 = 0;
...
static NEOERR * _builtin_bitmap_value_ex(CSPARSE *parse, CS_FUNCTION *csf, CSARG *args,
		CSARG *result)
{
	NEOERR *err;
	...
	err = cs_arg_parse(parse, args, "lii", &n1, &start, &num);
	if (err)
	{
		return nerr_pass(err);
	}

	result->n = cal_bitmap_result_value_ex(n1, start, num);

	return STATUS_OK;
}

static NEOERR * _builtin_bitmap_value(CSPARSE *parse, CS_FUNCTION *csf, CSARG *args,
		CSARG *result)
{
	NEOERR *err;
	unsigned long long n1 = 0;
	...
	err = cs_arg_parse(parse, args, "ll", &n1, &n2);
	if (err) return nerr_pass(err);

	result->n = cal_bitmap_result_value(n1 & n2);
...
static NEOERR *cs_init_internal (CSPARSE **parse, HDF *hdf, CSPARSE *parent)
{
	...
	if (parent == NULL)
	{
		static struct _builtin_functions {
			...
			{ "abs", 1, _builtin_abs },
			{ "bitmap_value", 2, _builtin_bitmap_value},
			{ "bitmap_value_ex", 3, _builtin_bitmap_value_ex},
			{ "bit_and", 2, _builtin_bit_and},
			{ "bit_or", 2, _builtin_bit_or},
			{ "html_encode", 2, _builtin_html_encode},
			{ "html_decode", 2, _builtin_html_decode},
			{ "json_encode", 2, _builtin_json_encode},
			{ "json_decode", 2, _builtin_json_decode},
			{ "uri_encode", 1, _builtin_uri_encode},
			{ "uri_decode", 1, _builtin_uri_decode},
			{ "url_replace_4py", 1, _builtin_url_replace_4py},
			{ "u32_mod", 2, _builtin_u32_mod},
			{ "max", 2, _builtin_max },
			{ "min", 2, _builtin_min },
